...
{
    // The scene update doesn't have to wait on the frame fence.
    update_scene();
    ...
    // All descriptors allocated during the frame can be cleared now.
...
void VulkanEngine::init_descriptors()
{
...

    {
        ...
        // UNIFORM_BUFFER is chosen over SSBO, as this is a very small buffer and used as global object -> no overhead.
...
AllocatedImage VulkanEngine::create_image(void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
    ...
    // As staging image it needs TRANSFER_DST and TRANSFER_SRC.
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine)
{
    ...
    if (!vkutil::load_shader_module("../shaders/mesh.frag.spv", engine->_device, &meshFragShader)) {
        ...
        throw std::exception("Error building shader");
    }
    ...
    // the binding corresponds to the binding in the shader (binding = x)
